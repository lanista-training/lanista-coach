...
export default ({
  ...
  const [countryListValue, setCountryListValue] = useState(null);
  useEffect(() => {
    countryListValue && country != countryListValue.code && setCountry(countryListValue.code);
  }, [countryListValue]);
  useEffect(() => {
    country && setCountryListValue(countries.find( element => element.code == country ));
  }, [country]);

  return (
    <div className="section profile-address-section" id="section-address">
      <div className="section-header">{t( "address" )}</div>
      <StyledCard>
